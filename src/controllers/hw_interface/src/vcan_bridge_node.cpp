...
//     // Short timeout: we only get here after the status check above already
//     // confirmed rx_pending > 0, so the device has data ready and this
//     // should return almost immediately. If it does hit the timeout, the
//     // data is still pending on the device and will be picked up on the
//     // next poll (no loss - see the LIBUSB_ERROR_TIMEOUT handling below).
//     int rc = libusb_bulk_transfer(handle_, ep, rx_data.data(), rx_buffer_size, &actual, 20);
...
//                        SocketCanPort& vcan1, double poll_ms) {
...
//   // Default lowered from 20ms (50 Hz) to 2ms (~260 Hz measured), then to
//   // 0.5ms here: at 2ms, most of each ~3.5ms loop period was this fixed
//   // sleep rather than the ~1.5ms of actual USB status/data transactions,
//   // capping throughput well below what the adapter and USB bus can sustain.
//   // Configurable via the "poll_ms" ROS parameter (see main()) if profiling
//   // shows this can go lower (or needs to go higher to reduce USB/CPU load).
...
//   auto node = std::make_shared<rclcpp::Node>("vcan_bridge_node");
//   const double poll_ms = node->declare_parameter<double>("poll_ms", 0.5);
...
// Standalone C++ port of vcan_bridge.py.
//
// Bridges a CANalyst-II USB-CAN adapter (2 physical channels) to two Linux
// SocketCAN vcan interfaces (vcan0, vcan1), so the rest of the stack can
// talk to vcan0/vcan1 exactly as it does today.
//
// This file is intentionally NOT wired into any CMakeLists.txt/package.xml/
// launch file. Build and run it manually, e.g.:
//
//   g++ -std=c++17 -O2 -o vcan_bridge vcan_bridge.cpp -lusb-1.0 -lpthread
//   sudo VCAN_SUDO_PASSWORD=yourpassword ./vcan_bridge
//
