...
from motor_configure.motor_driver import (
    REG_MST_ID,
    REG_ESC_ID,
    flush_bus,
    probe_motor,
...
def api_set_channel():
    ...
    new_channel = str(body.get("channel", "")).strip()
    if not new_channel:
        return jsonify({"ok": False, "log": ["Nothing to do -- no channel given."]}), 400

    with bus_lock:
        old_bus = state["bus"]
        try:
            new_bus = can.interface.Bus(channel=new_channel, interface="socketcan")
        except can.CanInterfaceNotImplementedError as e:
            return jsonify({"ok": False, "log": [f"Failed to open '{new_channel}': {e}"]}), 400
        except OSError as e:
            return jsonify({"ok": False, "log": [
                f"Failed to open '{new_channel}': {e}",
                "Make sure the interface exists and is up, e.g.:",
                f"  sudo ip link add dev {new_channel} type vcan && sudo ip link set up {new_channel}",
            ]}), 400
    ...
    return jsonify({"ok": True, "channel": new_channel, "log": [f"Switched to channel '{new_channel}'."]})
...
def api_enable():
    ...
    motor_id = int(body["id"])
    with bus_lock:
        enable(state["bus"], motor_id)
    return jsonify({"ok": True, "log": [f"CAN id {motor_id}: enabled."]})
...
def api_disable():
    ...
    motor_id = int(body["id"])
    with bus_lock:
        disable(state["bus"], motor_id)
    return jsonify({"ok": True, "log": [f"CAN id {motor_id}: disabled."]})
