...
class TcpServer(Node):
    ...
    def listen_loop(self):
        ...
        try:
            ...
            # This runs on a background thread, so an unhandled error here would
            # kill the listener while the node kept spinning - looking healthy but
            # silently accepting nothing. Report it loudly instead.
            self.logerr(
                "Could not start TCP server on {}:{} - {}. "
                "Is another endpoint already running on this port?".format(
                    self.tcp_ip, self.tcp_port, e
                )
            )
            tcp_server.close()
            return
            ...
            while not self.shutdown_event.is_set():
                try:
                    ...
                    if self.shutdown_event.is_set():
                        break
                    # e.g. running out of file descriptors: log and keep serving
                    # rather than tearing the listener down.
                    self.logerr("Error accepting connection: {}".format(e))
                    continue

                try:
                    ClientThread(conn, self, ip, port).start()
                except Exception as e:
                    self.logerr("Could not start client thread for {}: {}".format(ip, e))
                    conn.close()
        finally:
            tcp_server.close()
            self.loginfo("TCP server stopped.")
    ...
    def handle_syscommand(self, topic, data):
        ...
        if function is None:
            self.logwarn("Unknown SysCommand '{}' - ignoring.".format(topic))
            self.send_unity_error("Don't understand SysCommand.'{}'".format(topic))
            return

        # Decoding and parsing must be guarded too: a truncated or corrupt
        # payload from Unity would otherwise raise straight out of the client
        # thread and drop the connection.
        try:
            message_json = data.decode("utf-8").rstrip("\x00")
            params = json.loads(message_json)
        except (UnicodeDecodeError, ValueError) as e:
            self.logerr("Malformed payload for SysCommand '{}': {}".format(topic, e))
            self.send_unity_error("Malformed payload for SysCommand '{}'".format(topic))
            return

        if not isinstance(params, dict):
            self.logerr(
                "SysCommand '{}' expects a JSON object, got {}".format(topic, type(params).__name__)
            )
            self.send_unity_error("SysCommand '{}' expects a JSON object".format(topic))
            return
        ...
        try:
            ...
            self.send_unity_error("Bad arguments for SysCommand '{}'".format(topic))
        except Exception as e:
            self.logerr(
                "Error executing SysCommand '{}': {}: {}".format(topic, type(e).__name__, e)
            )
    ...
    def destroy_nodes(self):
        ...
        for table in (
            ...
            for ros_node in list(table.values()):
                # One node failing to shut down shouldn't strand all the others.
                try:
                    ros_node.destroy_node()
                except Exception as e:
                    self.logwarn("Error destroying node: {}".format(e))

        try:
            self.destroy_node()
        except Exception as e:
            self.logwarn("Error destroying server node: {}".format(e))
...
class SysCommands:
    ...
    def remove_subscriber(self, topic):
        """Called by Unity on disconnect to clean up a registered subscriber."""
        old_node = self.tcp_server.subscribers_table.get(topic)
        if old_node is not None:
            self.tcp_server.unregister_node(old_node)
            ...
            self.tcp_server.logwarn("RemoveSubscriber({}) — topic not found, ignoring.".format(topic))

    def remove_publisher(self, topic):
        """Called by Unity on disconnect to clean up a registered publisher."""
        old_node = self.tcp_server.publishers_table.get(topic)
        if old_node is not None:
            self.tcp_server.unregister_node(old_node)
            del self.tcp_server.publishers_table[topic]
            self.tcp_server.loginfo("RemovePublisher({}) OK".format(topic))
        else:
            self.tcp_server.logwarn("RemovePublisher({}) — topic not found, ignoring.".format(topic))
