...
class UnityTcpSender:
    ...
    def __init__(self, tcp_server):
        ...
        # how long to wait for Unity to answer a service request before giving up
    ...
    def send_unity_service_request(self, topic, service_class, request):
        ...
        resumed = thread_pauser.sleep_until_resumed(timeout=self.service_timeout)

        if not resumed:
            # Don't block an executor thread forever if Unity never answers.
            with self.srv_lock:
                self.services_waiting.pop(srv_id, None)
            self.tcp_server.logerr(
                "Timed out after {}s waiting for Unity to answer service '{}'.".format(
                    self.service_timeout, topic
                )
            )
            return None

        if thread_pauser.result is None:
            self.tcp_server.logerr("Empty response from Unity service '{}'.".format(topic))
            return None
        ...
        try:
            ...
            self.tcp_server.logerr(
                "Could not deserialize response from Unity service '{}': {}".format(topic, e)
            )
            return None
    ...
    def send_unity_service_response(self, srv_id, data):
        ...
        if thread_pauser is None:
            # The request already timed out, or Unity sent a response we never
            # asked for. Previously this raised KeyError out of the client
            # thread and dropped the connection.
            self.tcp_server.logwarn(
                "Received a service response for unknown request id {} - ignoring.".format(srv_id)
            )
            return
    ...
    def send_topic_list(self):
        if self.queue is not None:
            ...
            for item in topics_and_types:
                fallback = item[1][0].replace("/msg/", "/") if item[1] else ""
                if len(item[1]) <= 1:
                    # single type - use it directly
                    topic_list.types.append(fallback)
                else:
                    node = self.get_registered_topic(item[0])
                    # parse_message_name returns None on failure, and Unity
                    # expects a string for every topic, so fall back to the type
                    # ROS reported.
                    parsed = self.parse_message_name(node.msg) if node is not None else None
                    topic_list.types.append(parsed if parsed is not None else fallback)
...
# These are serialized with json.dumps(obj.__dict__), so the fields must be set
# on self - plain local assignments left __dict__ empty.
