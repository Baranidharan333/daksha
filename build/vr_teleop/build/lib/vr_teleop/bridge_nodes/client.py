...
class ClientThread(threading.Thread):
    ...
    def read_message(self, conn):
        ...
        if full_message_size > 0 and not data:
            # Raise rather than return None: the caller unpacks this into two
            # variables, so returning None would only turn a clear transport
            # failure into a confusing TypeError.
            raise IOError(
                "No data for a message size of {}, closing connection".format(full_message_size)
            )
    ...
    def process_message(self, destination, data):
        """
        Route one decoded message from Unity to its destination.

        Raises on malformed input; the caller reports and keeps the connection
        alive rather than dropping it for one bad message.
        """
        if self.tcp_server.pending_srv_id is not None:
            # we've been told that this message is a service request/response
            srv_id = self.tcp_server.pending_srv_id
            is_request = self.tcp_server.pending_srv_is_request
            # clear the flag first, so a failure below can't wedge every
            # subsequent message into being treated as a service message
            ...
            error_msg = (
                "Not registered to publish topic '{}'! Valid publish topics are: {} ".format(
    ...
    def run(self):
        ...
        try:
            while not halt_event.is_set():
                # Transport errors escape to the handler below and end the
                # connection; everything after this point is recoverable.
                destination, data = self.read_message(self.conn)

                try:
                    self.process_message(destination, data)
                except Exception as e:
                    # One malformed or unroutable message must not take down a
                    # working connection - report it and read the next one.
                    error_msg = "Error handling message for '{}': {}: {}".format(
            ...
            elif "Connection timed out" in err_str:
                self.tcp_server.logwarn("Client timed out - possible network issue.")
            else:
                self.tcp_server.logerr("Connection error: {}".format(e))
        except Exception as e:
            self.tcp_server.logerr(
                "Unexpected error on connection from {}: {}: {}".format(
                    self.incoming_ip, type(e).__name__, e
                )
            )
        finally:
            halt_event.set()
