...
class RosSubscriber(RosReceiver):
    ...
    def send(self, data):
        ...
        """
        # This runs as a ROS subscription callback; letting an exception escape
        # would take down the executor thread, so report and continue.
        try:
            self.tcp_server.send_unity_message(self.topic, data)
        except Exception as e:
            self.get_logger().error(
                "Failed to forward message on {} to Unity: {}".format(self.topic, e)
            )
        return self.msg
