...
class RosPublisher(RosSender):
    ...
    def send(self, data):
        ...
        # `data` is already-serialized bytes; rclpy publishes those directly.
        # A malformed payload from Unity shouldn't propagate up and drop the
        # whole connection, so report it and move on.
        try:
            self.pub.publish(data)
        except Exception as e:
            self.get_logger().error("Failed to publish message: {}: {}".format(type(e).__name__, e))
    ...
    def unregister(self):
        ...
        try:
            ...
            self.get_logger().warning("Error destroying publisher: {}".format(e))
        self.destroy_node()
