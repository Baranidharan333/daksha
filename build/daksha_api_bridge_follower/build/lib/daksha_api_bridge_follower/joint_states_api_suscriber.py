#!/usr/bin/env python3
"""
Subscribes to the global /joint_states topic and forwards each message to
the Daksha API as a joint_states event, so the actual joint feedback shows
up on the live dashboard/websocket.
"""
...
class JointStatesApiSubscriber(Node):

    def __init__(self):
        super().__init__("joint_states_api_subscriber")
        ...
        # Global topic name (leading slash) so this node picks up
        # /joint_states regardless of the namespace it is launched under.
        self.create_subscription(JointState, "/joint_states", self._on_joint_states, 10)

        self.get_logger().info(
            f"Forwarding /joint_states -> {self.api_url}/daksha/{self.robot_id}/joint_states"
        )
    ...
    def _post(self, payload, retry_on_auth_error):
        ...
        try:
            resp = self.session.post(
                f"{self.api_url}/daksha/{self.robot_id}/joint_states",
                json=payload,
                headers={"Authorization": f"Bearer {self.token}"},
                timeout=5,
            )
        except requests.RequestException as e:
            self.get_logger().warn(f"[post] joint_states failed: {e}")
            return
        finally:
        ...
        if resp.status_code != 200:
            self.get_logger().warn(f"[post] joint_states -> {resp.status_code}: {resp.text}")
