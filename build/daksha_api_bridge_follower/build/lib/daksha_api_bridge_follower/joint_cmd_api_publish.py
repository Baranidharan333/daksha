...
any joint_cmd / joint_commands event it receives as a ROS2 JointState on
the global /joint_cmd_api topic, so commands sent from the dashboard/API
can be consumed by the rest of the ROS graph.
"""
...
class JointCmdApiPublisher(Node):

    def __init__(self):
        super().__init__("joint_cmd_api_publisher")
        ...
        self.pub = self.create_publisher(JointState, "/joint_cmd_api", 10)
        ...
        self.get_logger().info(
            f"Forwarding {self.api_url} events (robot '{self.robot_id}') -> /joint_cmd_api"
        )
    ...
    def _handle_event(self, event):
        ...
        if event.get("action") not in ("joint_cmd", "joint_commands"):
            return
        ...
        self.get_logger().debug(f"[joint_cmd_api] published {len(msg.name)} joints")


def main(args=None):
    rclpy.init(args=args)
    node = JointCmdApiPublisher()
    rclpy.spin(node)
    node.destroy_node()
    rclpy.shutdown()
