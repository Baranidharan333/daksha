#!/usr/bin/env python3
"""
  -  Multi-Camera Web Monitor  (ROS 2 + Flask)
====================================================
Subscribes to the robot's ROS 2 camera topics and serves them as a single
web page (2x2 layout) on the device's IP, with the  control-room theme
and the scanning/lock overlay. Open the printed URL from any device on the
same network.

    ROBOT (with ROS 2):   python3 ros_web.py
    ANY LAPTOP (no ROS):  python3 ros_web.py --demo

Then browse to   http://<device-ip>:8141

EDIT THE `CAMERAS` LIST BELOW to match your exact topics (`ros2 topic list`).
The topics are streamed as-is; overlays are drawn in the browser.
...
CAMERAS = [
    ...
    ("LEFT ARM", "/right/camera/color/image_raw"),
    ("ZED · LEFT", "/zed/zed_node/left/color/rect/image"),
    ("RIGHT ARM", "/left/camera/color/image_raw"),
]
PORT = 8141
JPEG_QUALITY = 80
...
async function ihubEmergencyStop(){
  ...
  <div class="topbar">
    <div class="brand">
      <div class="mark"></div>
      <div><h1></h1><div class="sub">3-SYSTEM VLA RUNTIME</div></div>
    </div>
    <div class="status">
  ...
  <div class="foot"> PERCEPTION · LIVE CAMERA ARRAY</div>
...
# ------------------------------------------------------------- main --------
def main():
    ap = argparse.ArgumentParser(description=" multi-camera web monitor")
    ...
    if args.demo:
        start_demo()
        print(" web monitor  [DEMO MODE]")
    else:
        try:
            start_ros()
            print(" web monitor  [ROS 2]")
    ...
    ip = lan_ip()
    print(f"  open:  http://{ip}:{args.port}    (and http://{socket.gethostname().lower()}.local:{args.port})")
    for c in CAMS:
